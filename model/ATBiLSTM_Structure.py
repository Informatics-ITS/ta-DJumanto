...
class ATBiLSTM_Structure:
    def __init__(self, train, test, validation, max_length=300, vocab_size_structure=10000, embedding_dim=128, lstm_units=64, epochs=10, batch_size=32, is_mining=False, eps=0.5, min_samples=1, top_n_words=10, attention_dropout_rate=0.5):
    ...
    def __build_model(self, max_length, vocab_size_structure, embedding_dim=128, lstm_units=64):
        ...
        structure_attention_hidden, structure_weights = AttentionLayer(name_suffix='structure')(structure_lstm)
        structure_attention_hidden = Dropout(self.attention_dropout_rate)(structure_attention_hidden)
        
        combined = concatenate([structure_attention_hidden], name='concatenate')
        output = Dense(2, activation='softmax', name='classification_output')(combined)
        
        structure_hidden_state = Lambda(lambda x: x, name='structure_hidden_state')(structure_attention_hidden)
        structure_weights = Lambda(lambda x: x, name='structure_weights')(structure_weights)
        
        model = Model(inputs=[structure_input], 
                      outputs=[output])
    ...
    def build_model(self):
        ...
        self.model = self.__build_model(self.max_length, len(self.structure_vocab), self.embedding_dim, self.lstm_units)
        self.model.summary()
    ...
    def fit(self):
        ...
        self.model.save('./h5/ATBiLSTM_Structure_model.h5')
        return history
    ...
    def evaluate(self, fold):
        ...
        #save outputs
        with open(f'./outputs/predictions/evaluation_metrics_ATBiLSTM_structure_epoch-{self.epochs}_lemdim-{self.embedding_dim}_lstmdim-{self.lstm_units}_batch-{self.batch_size}_fold-{fold}.json', 'w') as f:
