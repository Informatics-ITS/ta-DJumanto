...
class BiLSTM_Structure_TFIDF:
    ...
    def __build_model(self, max_length, vocab_size_structure, embedding_dim=128, lstm_units=64):
        ...
        combined = concatenate([structure_lstm], name='concatenate')
    ...
    def build_model(self):
        ...
        _, self.structure_train, _, self.structure_vocab, _, self.structure_vectorizer = tfidf_character_vocab_gen(df_train,max_len=self.max_length, max_features_content=self.vocab_size_content, max_features_structure=self.vocab_size_structure)
    ...
    def evaluate(self, fold):
        ...
        #save outputs
        with open(f'./outputs/predictions/evaluation_metrics_BiLSTM_Structure_TFIDF_epoch-{self.epochs}_lemdim-{self.embedding_dim}_lstmdim-{self.lstm_units}_batch-{self.batch_size}_batch-{self.batch_size}_fold-{fold}.json', 'w') as f:
