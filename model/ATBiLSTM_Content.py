...
class ATBiLSTM_Content:
    def __init__(self, train, test, validation, max_length=300, vocab_size_content=10000, embedding_dim=128, lstm_units=64, epochs=10, batch_size=32, is_mining=False, eps=0.5, min_samples=1, top_n_words=10,attention_dropout_rate=0.5):
    ...
    def __build_model(self, max_length, vocab_size_content, embedding_dim=128, lstm_units=64):
        ...
        content_attention_hidden, _ = AttentionLayer(name_suffix="content")(content_lstm)

        
        content_attention_hidden = Dropout(self.attention_dropout_rate)(content_attention_hidden)
        combined = concatenate([content_attention_hidden], name='concatenate')
    ...
    def fit(self):
        ...
        self.model.save('./h5/ATBiLSTM_content_model.h5')
        return history
    ...
    def evaluate(self, fold):
        ...
        #save outputs
        with open(f'./outputs/predictions/evaluation_metrics_ATBiLSTM_content_epoch-{self.epochs}_lemdim-{self.embedding_dim}_lstmdim-{self.lstm_units}_batch-{self.batch_size}_fold-{fold}.json', 'w') as f:
